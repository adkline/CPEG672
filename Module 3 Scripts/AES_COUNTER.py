...
im = Image.open('dwayne.png')
image_info = [im.tostring(), im.size, im.mode]


cipher = AES.new(key_pad, AES.MODE_CTR, counter=secret)

dec_image = cipher.decrypt(image_info[0])

Image.frombytes(image_info[2], image_info[1], dec_image).save('output.png')
