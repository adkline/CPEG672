import urllib2, random, itertools, collections, math
...
traintri = trigrams(trainlower)
trainquad = quadgrams(trainlower)
trainrelfreq = rel_freq(trainquad)
trainrelfreq2 = rel_freq(trainbi)
trainrelfreq3 = rel_freq(traintri)
...
test_key = 'iucjftekpxhvnmboqradgwslyz'
...
while i < num_iterations:
    ...
    #for item in itertools.permutations(swaps,26):
    
    old_testkey = test_key
   
    if test_key in keys_tried:
        score = keys_tried[test_key]
    
    if test_key not in keys_tried:
        
        decrypted =  decrypt(loweronly, test_key, alphabet)
        decrypted_3grams = trigrams(decrypted)
        decrypted_ngramsrefreq= rel_freq(decrypted_3grams)
        
        scoretri = fitnessscore(decrypted_ngramsrefreq, trainrelfreq3)   
    
        decrypted_2grams = bigrams(decrypted)
        decrypted_2gramsrefreq= rel_freq(decrypted_2grams)
        
        scorebi = fitnessscore(decrypted_2gramsrefreq,trainrelfreq2)
        ## save the score 
       # score = scorequad + scorebi
        score = scorebi

        keys_tried[test_key] = scorebi + scoretri
    ...
    if score <=  score_old: 
        ...
        score_old=score
        print iteration
        subdecrypted = file('DecryptedSUB', 'a')
        subdecrypted.write("possible new key with score ")
        subdecrypted.write(str(score))
        subdecrypted.write('\n')
        subdecrypted.write(test_key)
        subdecrypted.write('\n')
        subdecrypted.write(decrypted)
        
        #firstfound = 0
        ## Chose two random locations and swap, then check score
        print "old key before swap ", test_key
        ...
        str1 = ord(test_key[i])-97
        str2 = ord(test_key[j])-97
    
        tempstr1 = chr(str1+65)
        tempstr2 = chr(str2+65)
            
        print tempstr1
        print tempstr2
        
        test_key = test_key.replace(chr(str1+97),tempstr2)
        test_key = test_key.replace(chr(str2+97),tempstr1)
        
        test_key= test_key.lower()
            
        print "new test key is",  test_key
        subdecrypted.close()  
