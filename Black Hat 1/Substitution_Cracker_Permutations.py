...
for alpha in range(ord('a'), ord('a')+26):
    ...
    for letters in range(ord('a'), ord('a')+26):
        if abs(frequency[chr(letters)] - normal_freqs[chr(alpha)]) < .016:
            ...
            tempstring = tempstring + chr(letters)
            # put the value into an array to sort later.
            tempvalues.append(abs(frequency[chr(letters)] - normal_freqs[chr(alpha)]))
            # add to dict
            temp_key_dict[chr(letters)] = abs(frequency[chr(letters)] - normal_freqs[chr(alpha)])
            
    tempvalues.sort()
    tempvalues = tempvalues[:4]
    ...
    for pos in range(0,len(tempvalues)):
      for char in range(0,len(tempstring)): 
        if temp_key_dict[tempstring[char]] == tempvalues[pos]:
            if tempstring[char] not in sortedkeys:
                #print "iteration"
                sortedkeys = sortedkeys + (tempstring[char])

    possible_keysets[(alpha)-97] = sortedkeys
...
for testset in itertools.product(*possible_keysets):
    
  # print testset
    test_key = ''.join(testset)
    
    for alpha in range(ord('a'), ord('a')+26):
        repeated = 0
        checking = alpha
        for testkeyelement in range(0, len(test_key)):
            if ord(test_key[testkeyelement]) == alpha:
                repeated = repeated +1
        if repeated > 2:
            break
    ...
    if repeated <= 2:
        ...
        decrypted =  decrypt(loweronly[0:200], test_key, alphabet)
